...
export interface CallMetric {
  ...
  call_sid: string;
  routed_to: "dograh" | "vapi" | "textback"; // Where this call was routed
  timestamp: string;
...
export async function getCanaryMetrics(hoursBack: number = 24): Promise<{
  ...
  const vapiCalls = metricsArray.filter((m) => m.routed_to === "vapi").length;
...
> {
  ...
  metricsArray.forEach((m) => {
    ...
    const current = clientMap.get(clientId) || { total: 0, dograh: 0, vapi: 0, bookings: 0, errors: 0 };
