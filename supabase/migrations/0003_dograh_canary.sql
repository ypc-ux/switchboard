...
-- Switchboard: Migrating from Vapi to Dograh with parallel operation
...
-- Tracks calls routed to: dograh (new), vapi (current), textback (fallback)
CREATE TABLE IF NOT EXISTS call_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  client_id UUID NOT NULL REFERENCES clients(id),
  caller_number TEXT NOT NULL,
  call_sid TEXT NOT NULL,
  routed_to TEXT NOT NULL CHECK (routed_to IN ('dograh', 'vapi', 'textback')),
...
stats AS (
  SELECT
    ...
    COUNT(CASE WHEN routed_to = 'vapi' THEN 1 END) as vapi_calls,
